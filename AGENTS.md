...
Follow Kotlin conventions already used in `src/main/kotlin`: 4-space indentation, trailing commas where Kotlin supports them, and one top-level class or closely related set of declarations per file. Use `PascalCase` for classes, `camelCase` for functions and properties, and keep package names lowercase. Prefer descriptive names tied to IntelliJ concepts, for example `FuzzyFinderService` or `OpenFuzzyFinderAction`. Keep UI strings in `MyBundle.properties` instead of hardcoding them.
